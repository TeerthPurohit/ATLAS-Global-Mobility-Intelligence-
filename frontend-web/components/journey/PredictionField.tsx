...
export function PredictionField({ label, prediction, className, emphasis }: PredictionFieldProps) {
  const { value, unit, basis, reason } = prediction;
        ...
        {basis === "unavailable" ? (
          ...
          <span className="inline-flex w-fit items-center rounded-lg bg-parchment px-3 py-1.5">
            <span className="font-mono text-lg font-semibold text-[#1b1a17]">
              {value ?? "--"}
              {unit ? <span className="ml-1 text-xs font-normal text-[#4a4640]">{unit}</span> : null}
            </span>
          </span>
        ) : (
          <span
